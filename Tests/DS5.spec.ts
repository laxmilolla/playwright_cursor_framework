...
test.describe('Data Submissions', () => {
  test('DS5: Filter by Data Commons', async ({ page }) => {
    const row = getTestCase('DS5');
    if (!row) throw new Error('Test case DS5 not found in test-cases.json');
    ...
    await selectDropdownOptionByText(page, 'filterDataCommons', filterValue);
