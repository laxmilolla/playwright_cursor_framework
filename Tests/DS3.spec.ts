...
test.describe('Data Submissions', () => {
  test('DS3: Filter by Program', async ({ page }) => {
    const row = getTestCase('DS3');
    if (!row) throw new Error('Test case DS3 not found in test-cases.json');
    ...
    await selectDropdownOptionByText(page, 'filterProgram', filterValue);
