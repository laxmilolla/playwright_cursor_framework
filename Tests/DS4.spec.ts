...
test.describe('Data Submissions', () => {
  test('DS4: Filter by Status', async ({ page }) => {
    const row = getTestCase('DS4');
    if (!row) throw new Error('Test case DS4 not found in test-cases.json');
    ...
    await selectDropdownOptionByText(page, 'filterStatus', filterValue);
