...
test.describe('Data Submissions', () => {
  test('DS2: Filter by Submission Name', async ({ page }) => {
    const row = getTestCase('DS2');
    if (!row) throw new Error('Test case DS2 not found in test-cases.json');
    ...
    await getLocator(page, 'filterSubmissionName').fill(filterValue);
    await getLocator(page, 'filterSubmissionName').press('Enter');
